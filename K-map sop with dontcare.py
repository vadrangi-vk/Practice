class KMapSOP:
    def __init__(self, num_variables):
        ...
        self.kmap = [None] * self.size  # Initialize K-map with None (None means don't care)
        self.sop_expression = ''

    def set_values(self):
        """
        Allow user to input the truth values for the K-map, including don't care values.
        """
        print(f"Please input the truth table for {self.size} combinations (0, 1, or X for don't care).")
        print(f"Each entry corresponds to a combination of {self.num_variables} variables in binary order (from 000... to 111...).")

        for i in range(self.size):
            while True:
                try:
                    value = input(f"f({self.get_binary_representation(i)}) = ")
                    if value == 'X':
                        self.kmap[i] = 'X'  # Don't care value
                        break
                    elif value in ['0', '1']:
                        self.kmap[i] = int(value)
                        break
                    else:
                        print("Please enter either 0, 1, or  capital X.")
                except ValueError:
                    print("Invalid input. Please enter 0, 1, or capital X.")

    def get_sop_expression(self):
        """
        Generates the SOP expression by finding all the 1's in the K-map
        and then forming the sum of products, ignoring don't care cells ('X').
        """
        sop_terms = []
        for i in range(self.size):
            if self.kmap[i] == 1:
